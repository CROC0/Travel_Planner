            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                <div className="w-10 h-10 rounded-xl bg-[#00e5cc]/10 border border-[#00e5cc]/20 flex items-center justify-center">
                  <span className="text-[#00e5cc] font-bold text-base">{day}</span>
                </div>
                <h1 className="text-2xl font-bold text-white">
                  {dayPlan?.label ?? `Day ${day}`}
                </h1>
              </div>
              <p className="text-[#8888aa] text-sm">{formattedDate}</p>
            </div>

            <button
              onClick={() => resetDay(day)}
              className="flex items-center gap-1.5 text-xs text-[#8888aa] hover:text-red-400 transition-colors glass rounded-lg px-3 py-2"
              title="Reset day"
            >
