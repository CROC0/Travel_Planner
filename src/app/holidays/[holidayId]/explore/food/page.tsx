...
export default function FoodPage() {
  ...
  function handleAddToDay(day: number, spot: FoodSpot) {
    ...
    toast.success(`Added "${spot.name}" to Day ${day}`);
  }
  ...
  return (
    <PageWrapper>
      <div className="max-w-5xl mx-auto px-4 py-6 sm:py-10">
        ...
        <SectionHeading title="Food & Drink" subtitle={`${filtered.length} places to eat`} accent="gold" symbol="✦" />
        <CategoryFilter options={categories} active={active} onChange={setActive} />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((s) => (
            <FoodCard key={s.id} spot={s} dayCount={itinerary.length} onAddToDay={handleAddToDay} />
          ))}
        </div>
      </div>
    </PageWrapper>
