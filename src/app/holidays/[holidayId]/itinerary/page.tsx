...
export default function ItineraryPage() {
  ...
  const { itinerary, loading } = useItinerary(holiday.id, holiday);
  ...
  return (
    <PageWrapper>
      <div className="max-w-2xl mx-auto px-4 py-6 sm:py-10">
        <SectionHeading
          title="Itinerary"
          subtitle={`${holiday.name} · ${itinerary.length} days`}
          accent="teal"
          symbol="≡"
        />
        {loading ? (
          ...
          <>
            ...
            <div className="space-y-3">
              {itinerary.map((day) => (
                <DayCard key={day.day} dayPlan={day} holidayId={holiday.id} />
              ))}
            </div>
          </>
        )}
