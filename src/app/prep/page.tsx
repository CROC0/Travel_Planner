...
function TodoRow({
  ...
  return (
    <div
      className="flex items-center gap-3 p-3 sm:p-4 rounded-xl glass group transition-all duration-200"
      style={{ border: `1px solid ${item.completed ? 'rgba(255,255,255,0.04)' : `${meta.color}22`}` }}
    >
      {/* Checkbox */}
      <button
        onClick={onToggle}
        ...
        style={{
          borderColor: item.completed ? meta.color : `${meta.color}66`,
          background: item.completed ? meta.color : 'transparent',
        }}
      >
      ...
      {editing ? (
        <input
          ...
          onBlur={commitEdit}
          autoFocus
          className="flex-1 bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-sm text-white focus:outline-none focus:border-[#00e5cc]"
        />
      ) : (
        <span
          className="flex-1 text-sm leading-snug transition-colors"
          style={{ color: item.completed ? '#555577' : '#ddddee', textDecoration: item.completed ? 'line-through' : 'none' }}
        >
          {item.text}
        </span>
      )}
...
export default function PrepPage() {
  const { todos, loading, completed, total, addTodo, toggleTodo, deleteTodo, editTodo } = useTodos();
  ...
  const visible = (list: typeof todos) =>
  ...
  return (
    <PageWrapper>
      <div className="max-w-2xl mx-auto px-4 py-6 sm:py-10">
        <SectionHeading
          title="Trip Prep"
          subtitle="Everything to sort before you fly"
          accent="gold"
          symbol="◎"
        />
          ...
          <div className="glass rounded-2xl p-4 mb-6" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div className="h-1.5 rounded-full bg-white/5 overflow-hidden">
              <div
                ...
                style={{ width: `${pct}%`, background: pct === 100 ? '#10b981' : '#ffd700' }}
              />
            </div>
            {pct === 100 && (
              <p className="text-[#10b981] text-xs mt-2 text-center font-medium">All done — you&apos;re ready! 🎉</p>
            )}
          </div>
        )}
        ...
        <div className="glass rounded-2xl p-3 sm:p-4 mb-4" style={{ border: '1px solid rgba(255,255,255,0.08)' }}>
          <div className="flex gap-2 mb-3">
            <input
              ...
              className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white placeholder-[#8888aa] focus:outline-none focus:border-[#ffd700] transition-colors"
            />
            <button
              onClick={handleAdd}
              disabled={!input.trim()}
              className="px-4 py-2.5 rounded-xl text-sm font-semibold disabled:opacity-40 transition-all"
              style={{ background: 'linear-gradient(135deg, #ffd700, #f59e0b)', color: '#0a0a0f' }}
            >
              Add
            </button>
          </div>
        ...
        {total > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4">
            <button
              ...
              style={filter === 'all'
                ? { background: '#ffffff15', color: '#fff', border: '1px solid rgba(255,255,255,0.15)' }
                : { color: '#555577', border: '1px solid rgba(255,255,255,0.06)' }}
            >
              All
            </button>
          ...
          <div className="space-y-2 mb-6">
            {visible(pending).length === 0 && total === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-[#8888aa] text-sm">No tasks yet — add something above</p>
              </div>
            )}
